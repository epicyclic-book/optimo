...
config = {
  ...
  'num_obj': 14,
  'num_constr': 15,
  'bounds_min': [20, 20,  80, -1.0, -1.0, -1.0, 2, 2],
  'bounds_max': [40, 40, 100,  1.0,  1.0,  1.0, 6, 8],
  'ref_data': [31, 29, 91, 0, 0, 0, 4.23, 5],
...
def Problem(v, obj, con):
  ...
  AP = math.sqrt(math.pow(r_a1_dash, 2) - math.pow(r_b1, 2)) - r_w1 * math.sin(alpha_wt1)
  BP = math.sqrt(math.pow(r_a2_dash, 2) - math.pow(r_b2, 2)) - r_w2 * math.sin(alpha_wt1)
  LF = math.sqrt(math.pow(r_a2_dash, 2) - math.pow(r_b2, 2))
  KE = math.sqrt(math.pow(r_a3_dash, 2) - math.pow(r_b3, 2))
  MP = r_w2 * math.sin(alpha_wt1)
  NP = r_w1 * math.sin(alpha_wt1)

  LP = math.sqrt(math.pow(r_w2, 2) - math.pow(r_b2, 2))
  ...
  gamma_epc1 = 2 * v_t2 * math.sin(alpha_wt1)
  ...
  gamma_1max = (math.tan(alpha_at2) - math.tan(alpha_wt1)) * (i1 + 1) / \
               ((i1 + 1) * math.tan(alpha_wt1) - math.tan(alpha_at2))
  gamma_2max = (math.tan(alpha_at1) - math.tan(alpha_wt1)) * ((i1 + 1) / i1) / \
               (((i1 + 1) / i1) * math.tan(alpha_wt1) - math.tan(alpha_at1))
  ...
  ro_1a = (NP + AP) / math.cos(beta_b)
  ro_1r = (NP - BP) / math.cos(beta_b)
  ro_2a = (MP - AP) / math.cos(beta_b)
  ro_2r = (MP + BP) / math.cos(beta_b)
  ro_1a_dash = LE / math.cos(beta_b)
  ro_1r_dash = LF / math.cos(beta_b)
  ro_3a = KE / math.cos(beta_b)
  ro_3r = KF / math.cos(beta_b)

  F_na_dash  = 1000 * T2 / (math.sqrt(math.pow(r_b2, 2) + math.pow(AM, 2)) *
                            math.cos(alpha_t) * b * n_w)
  F_nr_dash  = 1000 * T2 / (r_a2_dash * math.cos(alpha_t) * b * n_w)
  ...
  sigma_F1 = F_n2 * sigma_F
  sigma_F2 = F_n1 * sigma_F

  con.append(400 - sigma_F1)
  con.append(400 - sigma_F2)

  # constraint 10-11: contact stress
  ...
  sigma_H1 = (Z_M * Z_beta / (K_HL * Z_L * Z_R * Z_V1 * Z_W * K_HX)) * \
             math.sqrt(K_Hbeta * K_V1 * K_O) * S_H * 9.81
  sigma_H2 = (Z_M * Z_beta / (K_HL * Z_L * Z_R * Z_V2 * Z_W * K_HX)) * \
             math.sqrt(K_Hbeta * K_V2 * K_O) * S_H * 9.81

  sigma_H1 = math.sqrt((F_t1 * (i1 + 1)) / (d_w2 * b * i1)) * Z_H1 * Z_eps1 * sigma_H1
  sigma_H2 = math.sqrt((F_t2 * (i2 - 1)) / (d_w1 * b * i2)) * Z_H2 * Z_eps2 * sigma_H2

  con.append(2000 - sigma_H1)
  con.append(2000 - sigma_H2)

  # constraint 12-13: involute interference
  ...
  # constraint 14: tip interference
